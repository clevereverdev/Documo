...
import { NotificationProvider } from '../context/NotificationContext';
import { UserAvatarProvider } from '../context/UserAvatarContext';
import { AuthUserProvider, useFirebaseAuth, useAuth } from "../firebase/auth"; // Adjust the path as needed
import '../public/styles/Payment.css'; // Reference the CSS file
import '../styles/globals.css';
...
function App({ Component, pageProps }) {
  const [showToastMsg, setShowToastMsg] = useState('');
  const [parentFolderId, setParentFolderId] = useState(null);
  const authUser = useAuth(); // Use your auth hook here
  ...
  return (
    <>
      ...
      <ParentFolderIdContext.Provider value={{ parentFolderId, setParentFolderId }}>
        <ShowToastContext.Provider value={{ showToastMsg, setShowToastMsg }}>
          <UserAvatarProvider userId={authUser?.uid}>
            {/* Wrap the component tree with NotificationProvider */}
            <NotificationProvider userId={authUser?.uid}>
              <Component {...pageProps} />
              <ToastContainer />
              {showToastMsg && <Toast msg={showToastMsg} />}
...
function NotificationWrapper({ children, userId }) {
  if (!userId) {
    return children; // Or handle accordingly, perhaps a loading indicator or redirect
  }
