...
function UploadFileModal({ closeModal }) {
  ...
  const handleNextClick = () => {
    if (sensitiveFound && !passwordVerified) {
      // If sensitive content is found and the password hasn't been verified, show a message or keep the password input visible.
      setResultMsg("Please enter the correct password to proceed.");
    } else {
      // If the password is verified or no sensitive content is found, proceed with the upload.
  ...
  const verifyPassword = () => {
    if (password === "secret") {
      ...
      setPasswordVerified(true); // Set the flag to true when the password is correct
    } else {
      setResultMsg("Incorrect password. Please try again.");
      setPasswordVerified(false); // Keep the flag false when the password is incorrect
    }
            ...
            sensitiveFound && !passwordVerified && (
              <div className="absolute inset-0 bg-black bg-opacity-50 flex justify-center items-center">
                <div className="p-4 bg-white rounded-lg text-center">
                  <p className="text-red-600">Sensitive content detected. Enter password to proceed.</p>
                  <input type="password" id="password" value={password} onChange={(e) => setPassword(e.target.value)} className="mt-2" />
                  <button onClick={verifyPassword} className="mt-2 bg-blue-500 text-white p-2 rounded hover:bg-blue-700">Submit</button>
                </div>
              </div>
            )
          }
